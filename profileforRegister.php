...
function getBannerPath($username) {
    ...
    $sql = "SELECT profileBanner FROM users WHERE username = :username";
    $stmt = $db->prepare($sql);
    $stmt->bindParam(':username', $username);
    $stmt->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES['profile_picture']['name'])) {
        ...
        $profile_picture_path = getProfilePicturePath($newUsername);
    }
    ...
    if (!empty($_FILES['banner']['name'])) {
        ...
        $banner_path = getBannerPath($newUsername);
    }
